...
class Encoder_stft(nn.Module):
    '''
    STFT encoder in torch
    '''
    def __init__(self, nfft, hop_length, win_length):
        super(Encoder_stft, self).__init__()
        self.nfft = nfft
        self.hop_length = hop_length
        self.win_length = win_length
        # self.channel_map = nn.Conv2d(2, 256, 1)
    def forward(self, x):
        if len(x.shape) == 3: # [B, C, T]
            ...
            C = 1
        stft = torch.stft(x, n_fft=self.nfft, hop_length=self.hop_length, win_length=self.win_length, center=False, return_complex=True).abs()
        stft = stft.reshape(-1, C, stft.shape[-2], stft.shape[-1])
        # stft = self.channel_map(stft)
        return stft
    
class Decoder_stft(nn.Module):
    '''
    STFT decoder in torch
    '''
    def __init__(self, nfft, hop_length, win_length):
    ...
    def forward(self, x):
        ...
        istft = torch.istft(x, n_fft=self.nfft, hop_length=self.hop_length, win_length=self.win_length, center=False)
        return istft
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels=1, kernel_size=2, out_channels=64):
...
class Dual_Path_RNN(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, hidden_channels,
        ...
        self.conv1d = nn.Conv2d(in_channels, out_channels, 1, bias=False)
        ...
        self.end_conv1x1 = nn.Conv2d(out_channels, in_channels, 1, bias=False)
        self.prelu = nn.PReLU()
        self.activation = nn.ReLU()
         # gated output layer
        self.output = nn.Sequential(nn.Conv2d(out_channels, out_channels, 1),
                                    nn.Tanh()
                                    )
        self.output_gate = nn.Sequential(nn.Conv2d(out_channels, out_channels, 1),
                                         nn.Sigmoid()
                                         )
...
class Dual_RNN_model(nn.Module):
    '''
       ...
       input:
            ...
            num_spks: the number of speakers
    '''
    def __init__(self, in_channels, out_channels, hidden_channels, microphone_channels=1,
                 kernel_size=2, rnn_type='LSTM', norm='ln', dropout=0,
                 bidirectional=False, num_layers=4, K=200, num_spks=2):
        super(Dual_RNN_model,self).__init__()

        # self.encoder = Encoder(in_channels=microphone_channels, kernel_size=kernel_size,out_channels=in_channels)
        # self.decoder = Decoder(in_channels=in_channels, out_channels=1, kernel_size=kernel_size, stride=kernel_size//2, bias=False)

        self.encoder = Encoder_stft(256, 256, 256)
        self.decoder = Decoder_stft(256, 256, 256)
    ...
    def forward(self, x):
        ...
        '''
        # [B, N, L]
        # region_active = x['region_active']; 
        x = x['Binaural']; 
        e = self.encoder(x)
        # [spks, B, N, L]
        s = self.separation(e)
        # print(len(s), s[0].shape, e.shape)
        # [B, N, L] -> [B, L]
        out = [s[i]*e[:, i] for i in range(self.num_spks)]
        audio = [self.decoder(out[i]) for i in range(self.num_spks)]
        audio = torch.stack(audio, dim=1)
        # audio *= region_active.unsqueeze(-1) # cheat, know the region before
        return audio
    
    def vis(self, pred, label, epoch):
        import matplotlib.pyplot as plt
        pred = pred[0].squeeze(); label = label[0].squeeze()
        fig, axs = plt.subplots(2, 1)
        axs[0].plot(pred.cpu().detach().numpy())
        axs[1].plot(label.cpu().detach().numpy())
