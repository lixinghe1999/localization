...
class SpatialEncoder(torch.nn.Module):
    def __init__(self, channels=[32, 64, 64, 128, 128, 256, 256]):
...
class SpatialDecoder(torch.nn.Module):
    def __init__(self, channels=[128, 128, 128, 128, 128, 256, 256], n_mics=7):
        ...
        self.n_mics = n_mics
        self.conv_list.apply(init_weights)
        self.conv_post = weight_norm(nn.Conv2d(128, n_mics*2*9*3, (5,5), (1,1), padding=(2,2)))
        self.conv_post.apply(init_weights)
    ...
    def forward(self, x):
        ...
        # print(x.shape)
        x = x.reshape(bs, self.n_mics, 2, 3, 9, n_frames, 321)
        # print(x.shape)
