...
#     def __init__(self, h, n_dim=1024):
#         super(Quantizer, self).__init__()
#         assert n_dim % h.n_code_groups == 0
#         self.residul_layer = 4
#         self.quantizer_modules = nn.ModuleList([nn.ModuleList([
#             Quantizer_module(h.n_codes, n_dim // h.n_code_groups) for _ in range(h.n_code_groups)
#         ]) for _ in range(self.residul_layer)])
...
#         x = xin.reshape(-1, self.n_dim)
#         x = torch.split(x, self.n_dim // self.h.n_code_groups, dim=-1)
#         min_indicies = []
#         z_q = []
#         # if idx == 0:   
...
class Quantizer(torch.nn.Module):
    def __init__(self, h, n_dim=1024, n_code_groups=6, residual_layer=2):
        super(Quantizer, self).__init__()
        assert n_dim % n_code_groups == 0
        self.quantizer_modules = nn.ModuleList([
            Quantizer_module(h.n_codes, n_dim // n_code_groups) for _ in range(n_code_groups)
        ])
        self.subsequent_quantizer_modules = nn.ModuleList([nn.ModuleList([
            Quantizer_module(h.n_codes, n_dim // n_code_groups) for _ in range(n_code_groups)
        ]) for _ in range(residual_layer-1)])
    ...
    def for_one_step(self, xin, idx):
        xin = xin.transpose(1, 2)
        x = xin.reshape(-1, self.n_dim)
        x = torch.split(x, self.n_dim // self.n_code_groups, dim=-1)
        min_indicies = []
        z_q = []
        if idx == 0:   
            ...
            for _x, m in zip(x, self.subsequent_quantizer_modules[idx-1]):
                _z_q, _min_indicies = m(_x)
                z_q.append(_z_q)
                min_indicies.append(_min_indicies) #B * T,
            z_q = torch.cat(z_q, -1).reshape(xin.shape)
        ...
        for i in range(self.residul_layer):
            ...
            if i == 0:
                ...
                for j in range(self.n_code_groups):
                    ...
                    embed = self.subsequent_quantizer_modules[i-1][j]
                    q = embed.embedding(q.squeeze(-1))
                    ret.append(q)
                ret = torch.cat(ret, -1)
                quantized_out = quantized_out + ret
