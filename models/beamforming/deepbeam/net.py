...
class ComplexSTFTWrapper(nn.Module):
    ...
    def transform(self, input_data):
        ...
        stft_complex=torch.stft(input_data, n_fft=self.win_length, hop_length=self.hop_length, center=self.center,
                                window=torch.hann_window(self.win_length, device=input_data.device), return_complex=True)
        r = stft_complex.real
        i = stft_complex.imag
        _,F,T=r.shape
        r = r.view(B,C,F,T)
    ...
    def reverse(self, input_data):
        ...
        r=r.flatten(0,1)
        i=i.flatten(0,1)
        input_data=torch.complex(r,i)
        r=torch.istft(input_data, n_fft=self.win_length, hop_length=self.hop_length, center=self.center,
                      window=torch.hann_window(self.win_length, device=input_data.device), return_complex=False) # B, L
        return r.view(B,C,-1)
        ...
        for i in range(len(self.synth_conv)):
            ...
            if ts is not None:
                if ts_sum is None:
                    ...
                    ts0=F.interpolate(ts[0], scale_factor=scale_factor)
                    ts1=F.interpolate(ts[1], scale_factor=scale_factor)
...
if __name__ == '__main__':
    model=BeamformerModel(ch_in=9, synth_mid=64, synth_hid=96, block_size=16, kernel=3, synth_layer=4, synth_rep=4, lookahead=0)
    print(model)
    x=torch.randn(2,9,40000-8)
    y = model(x)
    print(y.shape)