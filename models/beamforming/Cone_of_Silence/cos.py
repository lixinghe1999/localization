...
class CoSNetwork(nn.Module):
    ...
    def __init__(
            self,
            n_audio_channels: int = 5,  # pylint: disable=redefined-outer-name
    ...
    def preprocess(self, mix):
        mix, means, stds = normalize_input(mix)
        valid_length = self.valid_length(mix.shape[-1])
        delta = valid_length - mix.shape[-1]
        mix = F.pad(mix, (delta // 2, delta - delta // 2))
        return mix, means, stds
    def postprocess(self, output_signal, mix, means, stds):
        output_signal = center_trim(output_signal, mix)
        # Un-normalize
        output_signal = unnormalize_input(output_signal, means, stds)
        # output_voices = output_signal[:, 0]

        return output_signal
    def forward(self, mix: torch.Tensor):  # pylint: disable=arguments-differ
        """
        ...
        """ 
        padded, means, stds = self.preprocess(mix)
        ...
        x = self.postprocess(x, mix, means, stds)
        return x
