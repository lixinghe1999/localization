...
signal2 = pra.highpass(signal2, Fs) * 0.1
delay2 = 1.0
...
room1.compute_rir()
premix = room1.simulate(return_premix=True)
print(signal1.shape, signal2.shape, premix.shape)
wavfile.write(path + "/input_samples/signal1.wav", Fs, premix[0, mics.M // 2])
wavfile.write(path + "/input_samples/signal2.wav", Fs, premix[1, mics.M // 2])
...
# process the signal
output = mics.process()
print(mics.signals.shape, output.shape)
...
print(n_lim, input_clean.shape, input_mic.shape, out_DirectMVDR.shape)
print(si_snr(input_clean, input_mic))
print(si_snr(input_clean, out_DirectMVDR))

print(pesq(Fs, input_clean, input_mic, 'nb'), pesq(Fs, input_clean, out_DirectMVDR, 'nb'))
