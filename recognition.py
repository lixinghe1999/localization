...
class AudioRecognition(pl.LightningModule):
    def __init__(self, train_dataset=None, test_dataset=None, model_name='mn10_as', lr=1e-3,):
        super().__init__()
        self.label_level = 'frame'
        if train_dataset is None or test_dataset is None: # load default dataset
            dataset = AudioSet_dataset(root='dataset/audio/audioset', split='train', frame_duration=0.1, modality=['audio', 'embeddings'], label_level=self.label_level)
            self.train_dataset, self.test_dataset = random_split(dataset, [int(len(dataset)*0.8), len(dataset)-int(len(dataset)*0.8)])
            self.num_classes = dataset.num_classes

            # self.train_dataset = AudioSet_dataset(root=root, split='train', frame_duration=1, vision=False, label_level=self.label_level)
            # self.test_dataset = AudioSet_dataset(root=root, split='eval', frame_duration=1, vision=False, label_level=self.label_level)
            # self.train_dataset = FSD50K_dataset('dataset/audio/FSD50K', split='dev', label_level=self.label_level)
            # self.test_dataset = FSD50K_dataset('dataset/audio/FSD50K', split='eval', label_level=self.label_level)
        else:
        ...
        self.model = Sound_Event_Detector(model_name, self.num_classes, frame_duration=0.1)
        self.lr = lr
    ...
    def training_step(self, batch, batch_idx):
        # x = batch['audio']
        x = [batch['audio'], batch['embeddings']]
        y = batch['cls_label']
        y_hat = self.model(x)
    ...
    def train_dataloader(self):
        return torch.utils.data.DataLoader(self.train_dataset, batch_size=32, shuffle=True, num_workers=8)
