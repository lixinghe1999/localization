...
from .label import Gaussian, Distance, Cartesian, filter_label
import os
import librosa
import json
import numpy as np
...
class Main_dataset(Dataset):
    def __init__(self, dataset, config=None, sr=16000):
        ...
        print('Before filtering, we have {} labels'.format(len(self.labels)))
        self.labels = filter_label(self.labels, self.config['classifier']['max_azimuth'], self.config['classifier']['min_azimuth'])
        print('After filtering, we have {} labels'.format(len(self.labels)))
        self.sr = sr
        self.duration = self.config['duration']
        self.feature_folder =  os.path.join(self.root_dir, 'features')
    ...
    def pre_compute_feature(self,):
        ...
        for idx in tqdm(range(self.__len__())):
            ...
            # np.save(os.path.join(self.feature_folder, str(idx)), audio)
            np.savez(os.path.join(self.feature_folder, str(idx)), **audio)
    ...
    def __getitem__(self, idx, on_the_fly=False): 
        ...
        if on_the_fly:
            ...
            audio_features = np.load(os.path.join(self.feature_folder, label['fname'] + '.npz'), allow_pickle=True)
